...
// The following code will first disable the context menu item with ID "Neuroskibe" when the active tab is changed or reloaded.
...
function updateContextMenu(tab) {
  chrome.contextMenus.update("Neuroskibe", { enabled: false });
  ...
  chrome.tabs.sendMessage(tab.id, { greeting: "shouldReenableContextMenu" }, function (response) {
    if (response && response.farewell === "yes") {
      chrome.contextMenus.update("Neuroskibe", { enabled: true });
...
chrome.commands.onCommand.addListener(function (command) {
  ...
  if (contextMenuEnabled) {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      chrome.tabs.sendMessage(tabs[0].id, "Neuroskibe: Content script not yet running. Wait for the page to load.", function (response) {
...
chrome.contextMenus.onClicked.addListener(async (info, tabs) => {
  ...
  if (typeof APIKEY === "undefined") {
    chrome.tabs.sendMessage(
      tabs.id,
      "Neuroskibe: \n API KEY not found. Click on the extension icon to set it. \n (Top right corner of the browser -> puzzle piece icon -> Neuroskibe)"
    );
    return;
  }
